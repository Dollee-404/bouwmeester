...
function tagConfig(reden: WerkvoorraadReden): { label: string; className: string } {
  switch (reden.type) {
    ...
    case "start-vandaag":
      return { label: "start vandaag", className: "text-amber-600" };
    case "start-morgen":
      return { label: "start morgen", className: "text-slate-400" };
    case "start-dag":
      return { label: `start ${reden.dag}`, className: "text-slate-400" };
    case "klaar-voor":
      return { label: `klaar vóór ${reden.dag}`, className: "text-amber-600" };
    case "mijlpaal":
      return {
        label: reden.overDagen === 0 ? "mijlpaal vandaag" : `mijlpaal over ${reden.overDagen}d`,
...
function WerkvoorraadRij({
  ...
  const tag = tagConfig(item.reden);
  ...
  if (items.length === 0) {
    return (
      <div className="mt-6 py-8 text-center">
        <p className="text-sm text-slate-400">Geen aandachtspunten voor deze week.</p>
      </div>
    );
  }
